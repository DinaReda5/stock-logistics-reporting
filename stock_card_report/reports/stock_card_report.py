...
class StockCardView(models.TransientModel):
  ...
  def _compute_display_name(self):
    for rec in self:
      rec.display_name = (f"{rec.reference} ({rec.picking_id.origin})"
                          if rec.picking_id.origin else rec.reference)
...
class StockCardReport(models.TransientModel):
  ...
  def print_report(self, report_type="qweb"):
    self.ensure_one()
    action = (report_type == "xlsx" and
              self.env.ref("stock_card_report.action_stock_card_report_xlsx") or
  ...
  def _get_html(self):
    ...
    if not active_id:
      return result
    try:
      # Use env to browse since self might be an empty recordset when called as model method
      report = self.env["report.stock.card.report"].browse(active_id)
      ...
      _logger.error("Error rendering stock card report HTML: %s", str(e), exc_info=True)
      return result
    return result

  @api.model
  def get_html(self, given_context=None):
    context = given_context or {}
    if not context.get("active_id"):
      return {"html": ""}
    return self.env["report.stock.card.report"].with_context(**context)._get_html()
